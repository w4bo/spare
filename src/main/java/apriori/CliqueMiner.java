...
public class CliqueMiner
    ...
    private void  printCliquerDebug(final String message, final int vertex) {
    	System.out.println("CLIQUER_DEBUG["+vertex+"]: " + message);
	}
...
//	     System.out.println(tuple._1 + "\t" + tuple._2);
	...
	while (true) {
		...
		for (final IntSet candidateItemset : candidate) {
			...
			for (final IntSet groundItemset : ground) {
				...
				if (simplifiedIntersectedTimestampSet.size() >= K) {
					...
					printCliquerDebug("Itemset " + newCandidateItemset + " is potentially viable for K ", starVertex._1);
	    ...
	    printCliquerDebug(" Object-Grow: " + level + ", "
		    + time_taken + " ms  cand_size:" + candidate.size(), starVertex._1);
//	    if(time_taken > 20000) {
//		break;
