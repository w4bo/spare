...
/**
 * Another functional wrapper for the Snapshot generation.
 */
public class SnapshotGenerator implements PairFunction<String, Integer, SnapShot> {
    ...
    public Tuple2<Integer, SnapShot> call(String t) throws Exception {
        ...
        p = new Point(Double.parseDouble(splits[1]), Double.parseDouble(splits[2])); //we need to new a point for every tuple,
        //the reference of the object will only be used once
        SnapShot sp = new SnapShot(ts);
        sp.addObject(obj_key, p);
        return new Tuple2<>(ts, sp);
    }
}