...
public class GeolifeTrajectoryTransform {
	...
	data_path = Paths.get(GeolifeTrajectoryTransform.class.getClass().getResource("/" + inputDir).getPath());
	try {
	    DirectoryStream<Path> ds = Files.newDirectoryStream(data_path);
	    FileWriter fw = new FileWriter(GeolifeTrajectoryTransform.class.getClass()
				.getResource(path_prefix + outputDir).getPath());
	...
	try {
		...
		double sumlg = allTPs.get(current).getLon();
		while (next < allTPs.size()) {
			...
			double eq_diffy = (allTPs.get(next).getLon() - allTPs
				.get(current).getLon()) / div;
			for (int i = 1; i < div; i++) {
			...
			sumlg = allTPs.get(current).getLon();
			next++;
		    } else {
			while (next < allTPs.size()
