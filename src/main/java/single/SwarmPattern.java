...
public class SwarmPattern implements PatternMiner {
    ...
    private int epsilon, minPoints, m, k;
    private ArrayList<SnapShot> input;
    private HashMap<Integer, IntSortedSet> obj_temporal;
    private ArrayList<ArrayList<Integer>> patterns;
    
    private ArrayList<Pair<ArrayList<Integer>, ArrayList<Integer>>> patternTemporal;
	...
	if (input == null) {
		...
		// DBSCANClustering
		DBSCANClustering dbscan = new DBSCANClustering(epsilon, minPoints, sp);
		int time = sp.getTS();
	...
	if (input == null) {
	    ...
	    for (SnapShot sp : input) {
	    	...
	    	//System.out.println("Snapshot " + sp.getTS() + " contiene " + sp.size() + " punti");
		// DBSCANClustering
		DBSCANClustering dbscan = new DBSCANClustering(epsilon, minPoints, sp);
		int time = sp.getTS();
		SnapshotClusters sclusters = new SnapshotClusters(time);
		...
		//System.out.println("Cluster trovato da DBSCAN: " + sclusters + " composto da " + sclusters.getClusterSize() + " elementi");
		clusters_snapshots.add(sclusters);
	    }
	    ...
	    // Object search starts from obj_temporal map
		for(SnapshotClusters snapshotClusters : clusters_snapshots) {
			System.out.println("\tSnapshot at time " + snapshotClusters.getTimeStamp() + " found the following clusters:");
			for(SimpleCluster cluster : snapshotClusters.getClusters()) {
				System.out.println("Cluster " + cluster.getID() + ": " + cluster.getObjects());
			}
		}
	    time_start = System.currentTimeMillis();
	    mineSwarm();
	...
	m = data[2];
	k = data[3];
	System.out.println("[SWARM]-Parameters: " + "e=" + epsilon + "\tp=" + minPoints
		+ "\tm=" + m + "\tk=" + k);
    }
