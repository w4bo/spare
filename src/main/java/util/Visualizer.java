...
public class Visualizer {
	...
	for (int i = 0; i < tr.size() - 1; i++) {
	    ...
	    if (i == 0) {
		StdDraw.point(tr.get(i).getLon(), tr.get(i).getLat());
		StdDraw.setPenColor(StdDraw.BLACK);
		StdDraw.text(tr.get(i).getLon(), tr.get(i).getLat() + delta,
			"<" + i + ">");
	    }
	    StdDraw.setPenColor(point);
	    StdDraw.point(tr.get(i + 1).getLon(), tr.get(i + 1).getLat());
	    StdDraw.setPenColor(StdDraw.BLACK);
	    StdDraw.text(tr.get(i + 1).getLon(), tr.get(i + 1).getLat()
		    + delta, "<" + (i + 1) + ">");

	    StdDraw.setPenColor(line);
	    StdDraw.setPenRadius(0.005);
	    StdDraw.line(tr.get(i).getLon(), tr.get(i).getLat(), tr.get(i + 1)
		    .getLon(), tr.get(i + 1).getLat());
	}
    }
}
